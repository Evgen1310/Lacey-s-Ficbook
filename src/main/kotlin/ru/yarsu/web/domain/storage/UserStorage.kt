...
class UserStorage(
    ...
    fun changeUserRole(user: User, roleNew: Int) {
        usersMap[user.login] = user.copy(role = roleNew)
    }

    fun changeUser(user: User, nicknameNew: String, passwordNew: String, aboutNew: String, roleNew: Int) {
    ...
    fun checkLogin(login: String?, ignore: String? = null): Boolean {
        val newLogins = logins.toList().toMutableList()
        newLogins.remove(ignore)
        return login in newLogins
    }

    fun checkNickname(nickname: String?, ignore: String? = null): Boolean {
